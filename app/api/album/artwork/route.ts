...
export async function POST(request:Request) {
 const parsed=z.object({ids:z.array(z.string().regex(/^[0-9]{1,20}$/)).min(1).max(20)}).safeParse(await boundedJson(request));
 ...
 for(const cover of covers){const {error}=await catalogWrite(user.id,"ding_set_itunes_artwork",{p_id:cover.id,p_url:cover.url});if(error)console.warn("artwork_persistence_failed",{code:error.code});}
